...
import { getMovies, deleteMovie, getLists, deleteMovieList, deletePlace,getPlaces } from "../../util/APIUtils";
import {
  Route,
  Redirect,
  Link
...
class PlaceList extends Component {
  constructor() {
    super();
    this.state = {
      data: this.loadPlaceList()
    };
    this.loadPlaceList = this.loadPlaceList.bind(this);
    ...
    clickHandler(mid){
        confirmAlert({
            ...
            buttons: [
              {
                label: 'Yes',
                onClick: () => deletePlace(mid).then((result) => {
                    this.loadPlaceList();
                }).catch(error => {
                  if(error.status === 404) {
                      this.setState({
                    ...
                    notification.error({
                      message: 'MovieApp',
                      description: "You can not delete a place which is have from users."
                  });  
                  }
              }) 
              },
  ...
  render() {
    ...
    return (
      <div className="pt-3">
        <h2 className="pb-2">Place List</h2>
        <ReactTable
          data={data}
          filterable
